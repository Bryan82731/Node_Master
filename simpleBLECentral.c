...
void scTaskAlertCallback(void) {
    ...
    if(rxbuf[0] == 0xAB && rxbuf[1] == 0xAB)//SlaveªºMAC¦^¶Ç
    {
      memcpy(MAC_ad,rxbuf+2,6);
      PIN_setOutputValue(hSbpPins, Board_UA2_GREEN, 1);
      MAC_flag = 1;//½T»{¦¬¨ìMAC Ãö³¬¨Æ¥ó
    }
    
    if((rxbuf[29]==0x22)&&(rxbuf[30]==0x22))//Åª¸g½n ¦¬ÀY§À
    {
      for(int i=0;i<20;i++)
        flash[i]=rxbuf[i];
...
static void SimpleBLECentral_taskFxn(UArg a0, UArg a1)
{
  ...
  GAPCentralRole_StartDiscovery(DEFAULT_DISCOVERY_MODE,
                              ...
                              DEFAULT_DISCOVERY_WHITE_LIST);   //¶}©l²Ä¤@¦¸±½´y
  ...
  for (;;)
  {
    ...
    if(events & CHECK_MAC_EVT)//¯Á¨úMAC event
    {
      events &= ~CHECK_MAC_EVT;
      
      if(MAC_flag == 1 && flash_flag == 1)//¦¬¨ì¤FMAC & lat lon
      {
        char buf[]="MAC_OK";
    ...
    case GAP_DEVICE_INFO_EVENT:
      {
        ...
        if(pEvent->deviceInfo.eventType == 0x04 || pEvent->deviceInfo.eventType == 0x00)//¥u±½ScanRsp©M¥þ¼s¼½
        {
          if(pEvent->deviceInfo.addr[0] != MAC_ad[5] || pEvent->deviceInfo.addr[1] != MAC_ad[4] ||
             ...
             pEvent->deviceInfo.addr[4] != MAC_ad[1] || pEvent->deviceInfo.addr[5] != MAC_ad[0] )//Á×¶}¦Û¤vSlaveªº¼s¼½ºÝ
          {
            //¦pªG²Å¦Xsmoke protocol¤¤ªºsmoke
            if(pEvent->deviceInfo.pEvtData[0]  == '$'   &&  pEvent->deviceInfo.pEvtData[25] == 0x00  &&
               ...
               pEvent->deviceInfo.pEvtData[30] == 0x00  )//¥Nªí°®²bªºsmole¸ê°T
            {
              //¤ñ¸û¦³µL­«½Æ
              for(int a=0;a<device_number;a++)
              {
                check_same = strncmp(pEvent->deviceInfo.pEvtData,device[a].data,25);//¥u¤ñ¹ï25§Y¥i,©¿²¤hopªÌ
                if(check_same != 0)//­«½Æ°¨¤W¤¤Â_
                  break;
              }
              ...
              if(check_same == 0)
              {
                memcpy(device[buffer_now_count_to].data,pEvent->deviceInfo.pEvtData,31);//«þ¨©
                ...
                device[buffer_now_count_to].data[29] = location_code;//°Ï½X Router ¥¼³B²z
                device[buffer_now_count_to].data[30] = MyHigh;
                device[buffer_now_count_to].have_data = 1;
                device[buffer_now_count_to].uart_to_slave = 1;//¥Nªí¶Ç°e§¹²¦
                
                scifUartTxPutChars(device[buffer_now_count_to].data,31);//¶Çµ¹Slave         
                ...
                next_buffer(&buffer_now_count_to);
                    ...
                    pEvent->deviceInfo.pEvtData[26] != 0x00  &&  pEvent->deviceInfo.pEvtData[27] != 0x00  &&
                    pEvent->deviceInfo.pEvtData[28] != 0x00  &&  pEvent->deviceInfo.pEvtData[29] != 0x00  &&
                    pEvent->deviceInfo.pEvtData[30] != 0x00     )//¥Nªí¬°hopping¸ê®Æ
            {
              //¤ñ¸û¦³µL­«½Æ
              for(int a=0;a<device_number;a++)
              {
                check_same = strncmp(pEvent->deviceInfo.pEvtData,device[a].data,31);//¥þ¤ñ¹ï
                if(check_same != 0)//­«½Æ°¨¤W¤¤Â_
                {
                  same_count = a;
                  break;
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:
      {
        ...
        for(int a=0;a<device_number;a++)
        {
          if(device[a].have_data == 1)//¥Nªí¦³¸ê®Æ ¶}©l­pºâ¸ê®Æ«OÅ@®É¶¡
            device[a].count++;
          
          if(device[a].protection == 1)//¥Nªí±N­n§â§R°£¸ê°T¶Ç¹L¥h
          {
          ...
          if(device[a].count > CLK && device[a].protection == 1)//¥i§R,¤S¸g¹LCLK«á ==>§R°£
          {
            for(int b=0;b<31;b++)
              device[a].data[b] = 0;
